...
function preload() {
  ...
  this.load.atlas('weapon', '../assets/weapon-anims.png', '../assets/weapon-anims.json'); // An atlas is a way to pack multiple images together into one texture. I'm using it to load all
...
function create() {
  ...
  player = this.physics.add.sprite(spawnPoint.x, spawnPoint.y, 'atlas', 'Character_Down.000.png').setSize(32, 32);
  weapon = this.physics.add.sprite(player.x, player.y, 'weapon', 'Sword_UpRight-3.png').setSize(32, 32).setOffset(0, -15);
  weapon.visible = false; //play invisible frame first
  ...
  this.input.keyboard.once('keydown_F', event => {
    ...
    console.log(game); // Create worldLayer collision graphic above the player, but below the help text
...
function update(time, delta) {
  let speed = 200;
  const prevVelocity = player.body.velocity.clone(); // console.log(player);
  ...
  weapon.depth = 0; // Horizontal movement
  ...
  function attack() {
    ...
    player.anims.currentAnim.hideOnComplete = true; // console.log(game.input.keyboard);
  ...
  if (cursors.space.isDown && player.body.facing === 13) {
    ...
    if (prevVelocity.x < 0) player.setTexture('atlas', 'Character_Left.000.png');else if (prevVelocity.x > 0) player.setTexture('atlas', 'Character_Right.000.png');else if (prevVelocity.y < 0) player.setTexture('atlas', 'Character_Up.000.png');else if (prevVelocity.y > 0) player.setTexture('atlas', 'Character_Down.000.png');
  }
}
